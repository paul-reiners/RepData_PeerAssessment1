...
getAvgStepsForInterval <- function(interval) {
    daily.activity[
        which(daily.activity[, "interval"] == interval, arr.ind=TRUE), "steps"]       
}
```
...
complete.activity$steps <- 
    ifelse(
        ...
        activity$steps)
complete.activity$steps <- ifelse(is.na(activity$steps), 0, activity$steps)
nrow(complete.activity) - sum(complete.cases(complete.activity))
